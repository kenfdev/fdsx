...
**Project context:** This is a TypeScript/Node.js package (`packages/fdsx-ui/`) using React, React Flow, Express, Vite, vitest, dagre, js-yaml. It lives inside a Python monorepo but is completely independent. **Do NOT read the repo root `CLAUDE.md`** — it is Python-specific.
...
- Style preferences beyond idiomatic TypeScript/React

**Don't:** Write code yourself, give vague feedback, review security or performance

## Review Checklist

**TypeScript Quality:** Proper typing (no unnecessary `any`), discriminated unions, strict null checks, correct use of generics
**React Quality:** Proper hook usage (dependency arrays, no stale closures), component composition, controlled vs. uncontrolled patterns
...
**Best Practices:** DRY, YAGNI, Fail Fast, idiomatic TypeScript/React, consistent style
**Anti-Patterns:** `any` type abuse, unused imports/variables, dead code, over-engineering, missing error boundaries

## Judgment

- **Blocking**: Bugs, DRY violations creating maintenance risk, unhandled error cases, dead code, code that contradicts the plan, `any` types where proper types are feasible
