...
- **Simple** (cheap model can handle): naming fixes, missing imports, unused variables, straightforward error handling, single-line logic fixes, adding simple tests, style/formatting, fixing `any` types with obvious replacements
- **Complex** (needs capable model): React hook dependency issues, type system rewrites (10+ errors), architectural refactors, security pattern changes, fixes that failed in previous iterations, complex generic type definitions
...
- The fix agent works best with specific, concrete instructions. Write exact code patterns, not vague guidance like "handle types properly"
