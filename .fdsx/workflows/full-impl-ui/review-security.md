...
**Project context:** This is a TypeScript/Node.js package (`packages/fdsx-ui/`) using React, React Flow, Express, Vite, js-yaml. It serves a local-only web UI for visualizing workflow YAML files. **Do NOT read the repo root `CLAUDE.md`** — it is Python-specific.

## Review Checklist

**Injection Prevention:** Command injection (child_process, exec), XSS (React dangerouslySetInnerHTML, unsanitized user input in templates), path traversal (fs operations with user-controlled paths)
**Server Security:** Express middleware (CORS, helmet), WebSocket origin validation, static file serving path containment
**Dependency Security:** Known vulnerable packages, unnecessary dependencies with large attack surface
**Data Protection:** Sensitive data in logs, error messages leaking internals to client
**AI-Generated Code:** AI-specific vulnerability patterns, dangerous defaults, overly permissive file access

**Don't:** Write code yourself, review design or code quality

## Judgment

- **Blocking**: Any security vulnerability that could be exploited (path traversal in file serving, command injection, XSS)
