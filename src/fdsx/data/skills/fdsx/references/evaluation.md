...
Material and question names match `[A-Za-z_][A-Za-z0-9_]*`. Each material has exactly one of `literal` or `ref`. References support concrete identifier fields, nonnegative array indices, and supported quoted object keys, not root/wildcards/filters. Missing references and immediate null fail before transmission. Empty strings (including whitespace-only strings), lists and objects are accepted by default; zero and false are also valid. In `evaluate` or `classifier`, set `allow_empty: false` on an individual material (for example `requirements: {ref: $.requirements, allow_empty: false}`) to require nonempty content. The option is a strict boolean, defaults to true, survives workflow serialization/resume, and works with either `ref` or `literal`. This replaces the former strict default; add explicit false overrides where existing workflows depend on rejection, including for resumed runs. Jev task prompts and question instructions still require nonblank text. Values retain their JSON types, and no policy flag is sent to Jev or LLM fallback. Nonempty containers may contain empty children. Nested values must be JSON-compatible with finite numbers and string keys. Literal content is not interpolated.
